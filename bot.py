...
TOKEN = "MTQ4NDUwNjk5OTAzODQxMDkzMg.GoTT2_.hrtAnk55ToHbI6xFyaQ6KSVX5Lb5ooTFxojSVE"
...
# --------------------------
# Atribuir Visitante ao entrar
# --------------------------

@bot.event
async def on_member_join(member):
    visitante = member.guild.get_role(VISITANTE_ID)
    if visitante:
        await member.add_roles(visitante, reason="Novo membro entrou")
...
ANONAO_ID = 1484568002396688545  # "não te interessa o meu ano"
...
WELCOME_CHANNEL_ID = 1483763425296515155  # <-- mete aqui o ID do canal 👋│boas-vindas
...
async def gerar_boas_vindas(member):
    ...
    # Fundo com gradiente verde → azul
    ...
    for i in range(450):
        cor = (
            int(40 + (i / 450) * 20),     # verde → azul (R)
            int(180 - (i / 450) * 80),    # verde → azul (G)
            int(120 + (i / 450) * 120),   # verde → azul (B)
            255
        )
    ...
    texto = (
        f"Olá {member.name}, chegastes de paraquedas então!\n"
